...
';
		...
		<?php
		/* rc_corephp - Adding check to tell user to set their live_site variable to the Joomla root */
		$JOOMLA_CONFIG = new JConfig();
		$__base = dirname( $base );
		$__root = substr( JURI::root(), 0, strpos( JURI::root(), $__base ) ).trailingslashit( $__base );
		if ( $__base && '/' != $__base && $JOOMLA_CONFIG->live_site != $__root ) { ?>
			<li><p><?php printf( 'Finally set the $live_site variable on your <code>configuration.php</code> file in <code>%s</code> to \'%s\'', realpath( JPATH_ROOT ), $__root ); ?></p></li>
		<?php } ?>
		</ol>
